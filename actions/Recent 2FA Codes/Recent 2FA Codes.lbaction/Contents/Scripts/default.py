...
def first_code_match(text):
    ...
    r = re.search("^\d{4,8}|(?<=[\s#])[0-9]{4,8}", text)
                ...
                #                 ^ code in the message but preceted by whitespace or #
...
# * that match multiple GLOBs (unfortunately Regex doesn't seem to be supported by sqlite on macOS Ventura.)
query = """
SELECT
    message.text,
